...
# For SEL planner
from utils.Planner.sel_planner import sel_plan
...
def summarize():
    ...
    try:
        ...
        if video_duration > max_duration:
            return jsonify({"error": "Video is longer than 1 hour"}), 400

        result = YT_Summarizer(video_url)
        
        # For no caption case
        if result == "No transcript available.":
            return jsonify({"error": "No captions available for this video. Summarization cannot be performed."}), 400
...
def generate_sel_plan_API():
    if request.method == 'POST':
        ...
        if not all([grade, sel_topic, learning_objectives, duration]):
            return jsonify({'error': 'Missing required field(s)'}), 400

        # Generate SEL plan
        response = sel_plan(grade, sel_topic, learning_objectives, duration)
