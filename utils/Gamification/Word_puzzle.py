...
def Word_puzzle(topic, numberofword):
    llm = ChatOpenAI(
        model="gpt-4o-mini",
    ...
    prompt_file_path = os.path.join('prompt_template', 'word_puzzal.txt')
    ...
    def prompt(topic, numberofword):
        prompt = prompt_template.replace("{topic}", topic).replace("{numberofword}", numberofword)
        try:
    ...
    output = prompt(topic, numberofword)
