...
def word_building_generation(difficulty_level, number_of_levels):
    llm = ChatOpenAI(
        model="gpt-4o-mini",
    ...
    prompt_file_path = os.path.join('prompt_template', 'Gamification', 'word_building.txt')
    ...
    def generate_word_building(difficulty_level, number_of_levels):
        prompt = prompt_template.replace("{difficulty_level}", difficulty_level).replace("{number_of_levels}", number_of_levels)
        try:
    ...
    output = generate_word_building(difficulty_level, number_of_levels)
