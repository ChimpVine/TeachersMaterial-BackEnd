...
def second_slide(response_first_slide):
    llm = ChatOpenAI(
        model="gpt-4o-mini",
    ...
    prompt_file_path=""
    prompt_file_path = os.path.join('prompt_template', 'Assessment', 'slide_two.txt')
    ...
    def generate_slide_2(response_first_slide):
        prompt = prompt_template.replace("{response_first_slide}", str(response_first_slide))
        try:
    ...
    output = generate_slide_2(response_first_slide)
