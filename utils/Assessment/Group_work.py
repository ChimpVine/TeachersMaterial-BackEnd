...
def Group_word(subject, grade, topic, learning_objective, group_size):
    llm = ChatOpenAI(
        model="gpt-4o-mini",
    ...
    prompt_file_path = os.path.join('prompt_template','Assessment', 'Group_word.txt')
    ...
    def prompt(subject, grade, topic, learning_objective, group_size):
        prompt = prompt_template.replace("{subject}", subject).replace("{grade}", grade).replace("{topic}", topic).replace("{learning_objective}", learning_objective).replace("{group_size}", group_size)
        try:
    ...
    output = prompt(subject, grade, topic, learning_objective, group_size)
